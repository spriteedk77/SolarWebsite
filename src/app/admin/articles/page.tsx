    ...
    <AdminShell signedIn>
      <Container width="wide"><div className="mx-auto w-full max-w-5xl py-10">
        <header className="flex flex-wrap items-end justify-between gap-5">
          ...
          <Link href="/admin/articles/new" className={adminPrimaryButton}>เขียนบทความใหม่</Link>
        </header>
        {(notice.published || notice.deleted) && <p role="status" className="mt-6 rounded-lg border border-green-200 bg-green-50 px-5 py-4 text-body font-medium text-green-800">{notice.deleted ? 'ลบบทความแล้ว' : 'เผยแพร่แล้ว'}</p>}
        {!result.ready ? (
          ...
          <ul className="mt-8 divide-y divide-hairline overflow-hidden rounded-card border border-hairline bg-white">
            {result.articles.map((article) => <li key={article.id} className="flex flex-wrap items-center justify-between gap-4 p-5 sm:px-6"><div><h2 className="text-body font-semibold text-navy-900">{article.title}</h2><p className="mt-1 text-caption text-ink-600">/{article.slug || 'ยังไม่มี-url'}</p><div className="mt-2 flex flex-wrap gap-2">{article.draft && <ContentStatusBadge status="draft" />}{article.published && <ContentStatusBadge status="published" />}</div></div><Link href={`/admin/articles/${article.id}`} className={adminSecondaryButton}>แก้ไข</Link></li>)}
          </ul>
        )}
      </div></Container>
    </AdminShell>
